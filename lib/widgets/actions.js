...
class Actions extends AbstractWidget {
  ...
  async makeOpen() {
    try {
      ...
      const mtx = await wallet.createOpen(this.state.selectedName, false);
  ...
  async makeReveal() {
    try {
      const wallet = this.state.getWallet();
      const mtx = await wallet.createReveal(this.state.selectedName);
      await this.app.signTX(mtx);
    } catch (e) {
      this.app.log(e.message);
    }
  ...
  async makeRedeem() {
    try {
      const wallet = this.state.getWallet();
      const mtx = await wallet.createRedeem(this.state.selectedName);
      await this.app.signTX(mtx);
    } catch (e) {
      this.app.log(e.message);
    }
