...
class Actions extends AbstractWidget {
    ...
    switch (ns.state) {
      ...
      case 'BIDDING':
        ...
        if (!ns.inWallet) {
          ...
          this.button2.content = 'RESCAN AUCTION';
          this.action2 = () => this.rescanAuction(ns);
        }
        return;
      case 'REVEAL':
  ...
  async rescanAuction(ns) {
    try {
      const wallet = this.state.getWallet();
      if (!wallet)
        return;

      await wallet.importName(ns.name);
      await this.app.wdb.rescan(ns.height - 1);
      await this.app.getSelectedWalletNames();
      await this.app.selectName(ns.name);
      this.hide(true);
    } catch (e) {
      this.app.error(e.message);
    }
