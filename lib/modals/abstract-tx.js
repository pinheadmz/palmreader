...
class AbstractTXModal extends AbstractModal {
  ...
  refresh() {
    ...
    // Used to generate file name when exported
    const description = {
      maxSigsNeeded: 0,
      recipientOrName: '',
      valueOrAction: ''
    ...
    for (let i = 0; i < mtx.inputs.length; i++) {
      ...
      if (witness) {
        ...
        sigsNeeded = m;
        if (sigsNeeded > description.maxSigsNeeded)
          description.maxSigsNeeded = sigsNeeded;
      }
    ...
    for (let i = 0; i < mtx.outputs.length; i++) {
      ...
      if (path) {
        ...
        // Probably a destination address
        description.recipientOrName = addr;
        description.valueOrAction = doosToHNS(output.value);
      }
      ...
      if (output.covenant.isName()) {
        ...
        description.recipientOrName = name;
        description.valueOrAction = action;
      }
    ...
    this.state.mtxFile =
        description.recipientOrName
      + '-'
      + description.valueOrAction
      + '-'
      + (description.maxSigsNeeded - 1) // Assume we are about to sign?
      + '_sigs_needed'
      + '-'
      + Date.now()
      + '.txt';
